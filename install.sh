...
wget -O /usr/bin/menu-backup "$sfile/menu-backup-auto-telegram.sh" && chmod 755 /usr/bin/menu-backup
if [ ! -s /usr/bin/menu-backup ]; then
    colorized_echo red "ERROR: menu-backup-auto-telegram.sh gagal di-download."
    exit 1
fi
bash -n /usr/bin/menu-backup || {
    colorized_echo red "ERROR: syntax menu-backup-auto-telegram.sh tidak valid."
    exit 1
}
