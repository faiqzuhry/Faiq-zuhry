...
# Jangan pernah membawa sertifikat lama ke instalasi domain baru.
# File ini akan dipakai Nginx CloudFront, jadi hapus target lama sebelum
# meminta sertifikat baru agar kegagalan ACME tidak diam-diam memakai cert lama.
rm -f /var/lib/marzban/xray.crt /var/lib/marzban/xray.key

# Minta sertifikat baru yang mencakup domain utama DAN host CloudFront.
/root/.acme.sh/acme.sh --server letsencrypt --register-account --issue \
    -d "$domain" -d "$CF_DOMAIN" --standalone -k ec-256 --debug
~/.acme.sh/acme.sh --installcert -d "$domain" \
    --fullchainpath /var/lib/marzban/xray.crt \
    --keypath /var/lib/marzban/xray.key --ecc

# Pastikan sertifikat yang benar-benar terpasang memuat kedua nama.
if ! openssl x509 -in /var/lib/marzban/xray.crt -noout -ext subjectAltName 2>/dev/null \
    | grep -Fq "DNS:${CF_DOMAIN}"; then
    echo "ERROR: Sertifikat tidak memuat CloudFront domain ${CF_DOMAIN}."
    exit 1
fi
if ! openssl x509 -in /var/lib/marzban/xray.crt -noout -ext subjectAltName 2>/dev/null \
    | grep -Fq "DNS:${domain}"; then
    echo "ERROR: Sertifikat tidak memuat domain ${domain}."
    exit 1
fi
echo "Certificate OK: ${domain} + ${CF_DOMAIN}"
