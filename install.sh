...
try:
    # Marzban v0.8.4 stores proxies in the separate `proxies` table;
    # `users.proxies` is an SQLAlchemy relationship, not a SQLite column.
    rows=con.execute("""
...
for r in rows:
    ...
    if ptype == "vmess":
        uid=settings.get("id")
        if uid and uid not in sv: vmess.append({"id":str(uid),"email":r["username"] or ""}); sv.add(uid)
    elif ptype == "vless":
        uid=settings.get("id")
        if uid and uid not in sl: vless.append({"id":str(uid),"email":r["username"] or ""}); sl.add(uid)
    elif ptype == "trojan":
