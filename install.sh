...
sed -i 's/regexp:\.\*$regex\.\*/regexp:.*${regex_routing}.*/g' /usr/bin/routing
...
routing_test_json() {
    ...
    if [ -x /var/lib/marzban/core/xray ]; then
        /var/lib/marzban/core/xray -test -config "$cfg" >/tmp/routing-xray-test.log 2>&1
        return $?
    fi
    ...
    docker exec "$container" /var/lib/marzban/core/xray -test -config /tmp/routing-xray-test.json >/tmp/routing-xray-test.log 2>&1
    local rc=$?
