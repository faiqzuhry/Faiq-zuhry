...
# - The public URL is the SAME domain with a dedicated path:
#       https://DOMAIN/cloudfront
#   so no second certificate/SAN or extra DNS hostname is required.
# ============================================================
CF_PORT=10010
CF_PATH="/cloudfront"
...
EOF
    ...
    # Dedicated CloudFront hostname. It never takes over the normal domain's "/".
    # Certificate is the same LE certificate created for the main domain.
...
EOF
    ...
    nginx -t || {
        echo -e "${RED}❌ Nginx CloudFront config gagal.${NC}"
        exit 1
    }
    ...
    echo -e "${GREEN}✓ CloudFront WS aktif.${NC}"
...
setup_host_nginx_cloudflare() {
    ...
    # Nginx only adds one dedicated Cloudflare path.
...
server {
    ...
    # Dedicated Xray Cloudflare path.
    # Cloudflare -> Nginx -> 127.0.0.1:${CF_PORT}
    location = ${CF_PATH} {
...
echo -e "${CYAN} Cloudflare Xray : https://${domain}${CF_PATH}"
echo -e "${CYAN} Cloudflare UUID : ${CF_UUID}"
echo -e "${CYAN} Xray-core       : ${XRAY_LATEST}"
echo -e "${CYAN} Backend         : 127.0.0.1:${CF_PORT}"
echo -e "${YELLOW} DNS Cloudflare  : arahkan ${domain} ke VPS dan aktifkan Proxy (orange cloud).${NC}"
